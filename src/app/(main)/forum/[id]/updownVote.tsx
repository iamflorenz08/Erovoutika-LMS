    ...
    return (
        <form
            action={async () => {
                ...
                setVoteDetails({
                    voteType: active,
                    voteCount: active === 'upvote' ? voteCount + 1 : active === 'downvote' ? voteCount - 1 : voteCount
                })
                await addVote(postId, active)
            }}
            className="flex gap-2 items-center">
            <button
                disabled={status === 'loading'}
                type="submit"
                onClick={() => voteDetails.voteType === 'downvote' || !voteDetails.voteType ? setActive('upvote') : setActive(undefined)}
            >
                {voteDetails.voteType === 'upvote' ? <PiArrowFatUpFill className='text-primary' size={24} /> : <PiArrowFatUpLight size={24} />}
            </button>
            <span>{voteDetails.voteCount}</span>
            <button
                disabled={status === 'loading'}
                type="submit"
                onClick={() => voteDetails.voteType === 'upvote' || !voteDetails.voteType ? setActive('downvote') : setActive(undefined)}
            >
                {voteDetails.voteType === 'downvote' ? <PiArrowFatDownFill className='text-primary' size={24} /> : <PiArrowFatDownLight size={24} />}
            </button>
        </form >
    )
}
