...
export default async function WatchedThreads() {
  ...
  const savedThreads: Array<IPost> = await getSavedThreads(
    ...
    <div className="hidden bg-white w-full max-w-[336px] rounded-lg p-4 xl:flex flex-col gap-4 h-fit sticky top-0 shadow-md">
      ...
      <div className="flex flex-col gap-4">
        {savedThreads && savedThreads.length > 0 ? (
          savedThreads.slice(0, 5).map((savedThread, index) => (
            <Link
              key={index}
            ...
            >
              ...
              />
              <div>
                <h1 className="font-medium text-[10px] text-gray capitalize line-clamp-1">
                  {savedThread.author?.fullName?.first}{" "}
                  {savedThread.author?.fullName?.last}
                </h1>
                <p className="font-semibold line-clamp-1">
                  {savedThread.title}
                </p>
                <h2 className="font-medium text-xs text-gray">
                  {savedThread.commentCount} comments
                </h2>
              </div>
            </Link>
          ))
